...
全章最重要的一节。形状和数字都可以自己打印出来核对：`python scripts/mla_shapes.py`
...
所以后面表里的「2048 → 3072」读作：**吃一个 2048 维向量，吐一个 3072 维向量**，
...
**这一步是 MLA 的精髓，也是最容易犯迷糊的地方**——存的时候压一下、用的时候再解开，
不是白折腾吗？接下来四小节专门回答这件事。
...
> `kv_b_proj` 这个「解压器」在 decode 时从来没有真正跑过，它被吸收进了 Q 侧和输出侧。**
...
| `kv_b_proj` | 512 → 4096 | **解压器**。从 latent 还原 16 个头的 K 和 V，4096 = 16 × (128 + 128)。decode 时被吸收，不真的执行 |
| `o_proj` | 2048 → 2048 | 16 个头各出 128 维，拼成 2048 投回主干道 |

**缓存的是 `kv_a_proj` 的输出（576 维），不是 `kv_b_proj` 的输出（4096 维）**，中间差了 7 倍多。
...
**为什么位置信息非要单独走一条通道？** 因为上面那条恒等式要求 `W_k` 能挪到 Q 那边，
而 RoPE 是跟位置相关的旋转，如果加在 latent 上，每个 token 转的角度都不同，`W_k` 就没法提出来合并，
MLA 省显存的优势也就没了。所以位置信息被剥出来单独存 64 维，16 个头共享同一份。
...
1. **为什么压缩**：decode 卡在 KV cache 带宽上，能省 8.9 倍，32k 上下文下并发从 7 条变 68 条。
2. **为什么能压**：16 个头的 K/V 都由同一个 h 线性生成，本来就冗余。
3. **为什么是 512 维低秩而不是直接存 h**：瓶颈和模型一起训练，比 h 还小 3.6 倍，参数量反而少 3.2 倍。
4. **解压的代价去哪了**：decode 靠矩阵吸收压根不解压；prefill 解压，但那里瓶颈是算力，解压代价线性可忽略。
5. **位置信息为什么单独走 64 维**：保证解压矩阵能被吸收，否则第 4 条不成立。
6. **完整版 V2 连 Q 也压缩**（`q_lora_rank=1536`），V2-Lite 省掉这步，`q_proj` 直接出结果。代码里两种都支持。
